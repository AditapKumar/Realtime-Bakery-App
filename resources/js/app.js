...
function removeCartItem(cake) {
  axios
    .delete("/api/remove-from-cart", { data: { _id: cake._id } })
    .then((res) => {
      cartCounter.innerText = res.data.totalQty;
...
if (container) {
  ...
  let cakeModel;
  loader.load("/models/chocolate_mousse_cake.glb", function (gltf) {
...
socket.on("orderStatusUpdate", (data) => {
  new Noty({
    type: "info",
    timeout: 3000,
    text: `Order #${data.orderId} status updated to: ${data.status}`,
    progressBar: false,
  }).show();
});
